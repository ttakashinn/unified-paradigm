...
Lưu ý phân biệt thuật ngữ rõ ràng để tránh nhầm lẫn:

- **“Ba dòng tư tưởng”** ở [Phần 0](index.md) là *lịch sử ý tưởng* (algebraic effects, actor model, Hollywood Principle) — đây là *nơi paradigm ra đời*.
- **“Bốn hiện thân”** ở các Phần 1-4 là *implementation cụ thể* (React, Elixir/OTP, IoC, Phoenix LiveView) — đây là *cách paradigm xuất hiện trong code thực tế*. LiveView là điểm hội tụ rõ nhất, kết hợp cả ba dòng vào một primitive UI.
- **“Ba trụ kiến trúc”** dưới đây là *kết quả triết học cụ thể* mà developer thấy trong code hàng ngày — đây là *cách paradigm biểu hiện ở mức nguyên lý*.

Đặt bốn hiện thân cạnh nhau, ta thấy chúng cùng đứng trên ba trụ:
...
1. *Handler* sống ở đâu? Lifetime của nó là gì? Có bao nhiêu handler nested?
1. *Failure model* là gì? (exception bubble, supervisor restart, replay từ event log, retry policy)
1. *Identity và lifetime* của state holder là gì? (unmount = lost? crash = lost? persistent?)
1. *Ranh giới pure/impure* được vẽ ở đâu? Có thể test pure phần riêng không?
...
- Chấp nhận imperative API (Web Animations, manual transaction control) bên trong callback của framework — đây không phải “phá vỡ IoC” mà là dùng đúng escape hatch khi pure declarative không đủ biểu đạt
...
Bản đồ thống nhất ở Phần 5 trình bày paradigm như một thắng lợi không có cost. Thực tế không phải như vậy. Mọi paradigm đều có ngữ cảnh mà nó tỏa sáng, và ngữ cảnh mà nó trở thành cản trở — Phần 6 sẽ chỉ ra cụ thể: khi nào IoC gây hại, khi nào nên giữ state local thay vì lift up, khi nào LiveView không phải lựa chọn, và bốn anti-pattern phổ biến của developer áp dụng paradigm như công thức thay vì như công cụ.
