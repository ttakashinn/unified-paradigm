...
defmodule ShoppingCart do
  ...
  # Registry là một process registry built-in của Elixir — cho phép đăng ký
  # process với tên có cấu trúc (tuple, struct...) thay vì atom đơn lẻ.
  # {:via, Registry, ...} tuple bảo OTP rằng "process này được đăng ký qua
  # Registry với key {:cart, user_id}" — cho phép có hàng triệu cart process
  # cùng lúc, mỗi cái lookup được bằng user_id.

  # ===== Server callbacks (chạy trong process GenServer) =====

  @impl true
  def init(init_arg), do: {:ok, init_arg, {:continue, :load_from_db}}
  # init/1 nhận init_arg (truyền từ start_link) và TRẢ VỀ initial state
  # — đây là điểm khác với React useState: init không phải state, init_arg là
  # input để tính ra state ban đầu

  @impl true
  def handle_continue(:load_from_db, state) do
    # Sau khi init xong, OTP gọi handle_continue để chạy initialization nặng
    # (load DB) ngoài hot path của caller — caller của start_link không bị block
...
defmodule MyApp.Application do
  ...
  def start(_type, _args) do
    children = [
      # Database pool — critical dependency, hầu hết children khác cần nó
      # Với :one_for_one ở root: nếu Repo crash thì CHỈ Repo restart (các
      # children khác giữ nguyên). Nếu muốn restart-everything-on-Repo-fail
      # phải dùng :rest_for_one và đặt Repo lên đầu list.
      {MyApp.Repo, []},
...
├── MyApp.Repo                    ← critical dependency, nếu crash chỉ Repo restart
...
defmodule MyApp.Accounts.User do
  ...
  def registration_changeset(user, params) do
    user
    |> cast(params, [:email, :name, :age])             # whitelist fields (không lấy :role từ user input)
    ...
    |> put_change(:role, :user)                        # mọi user đăng ký mặc định role :user
    |> unique_constraint(:email)                       # DB constraint (deferred — chỉ kiểm tra khi insert)
  end
