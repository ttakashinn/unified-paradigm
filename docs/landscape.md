...
| **Kotlin Coroutines + Structured Concurrency** | `CoroutineScope { }` enforce: child phải hoàn thành/cancel trước khi parent thoát. Tương đồng mạnh với OTP supervisor tree nhưng lifetime-scoped thay vì process-scoped. | OTP supervision trong JVM ecosystem, sans crash isolation |
