...
Unix pipe (`|`) là **inter-process message-passing thuần khiết**: stdin/stdout là mailbox; mỗi tiến trình là actor; init system (systemd/launchd) là supervisor — restart daemon khi crash, đảm bảo dependency order, manage lifecycle. Microservices, Kafka, Kubernetes pods là Unix philosophy mở rộng lên quy mô phân tán. Shell là framework điều phối dataflow; tiến trình của bạn là user code; shell quyết định fork, exec, pipe — bạn không control nó.
...
Landscape rộng đến mức này không phải là chứng cứ “bùng nổ” của paradigm — nó là dấu hiệu rằng đây là câu trả lời đúng cho một lớp bài toán cụ thể, được tái phát hiện độc lập trong nhiều cộng đồng kỹ thuật cách nhau hàng thập kỷ. Phần 8 cuối cùng là tài liệu đọc thêm: nguồn gốc các ý tưởng đã trình bày, được chia theo độ khó từ pragmatic engineering đến academic foundation, để bạn có thể đi sâu vào bất kỳ nhánh nào tuỳ theo ngữ cảnh và khẩu vị.
