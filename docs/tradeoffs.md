...
Phần 6 đã chỉ ra trade-offs trong phạm vi hai paradigm chính của tài liệu (React + Elixir/OTP). Nhưng paradigm “pure description + orchestrating runtime” không giới hạn ở UI và server backend — nó xuất hiện ở mọi tầng của stack hiện đại, từ Kubernetes đến game engine đến industrial control. Phần 7 zoom out để nhìn landscape toàn cục: cùng một thuật toán, ở rất nhiều quy mô và domain, được tái phát hiện độc lập vì nó là câu trả lời đúng.
